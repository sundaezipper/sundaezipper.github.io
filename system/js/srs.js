/* ==========================================================
   ...
   Robust version with fallback and better UI handling
   ========================================================== */
(() => {
  "use strict";
  ...
  --------------------------- */
  const config = {
    sheetUrl: "https://script.google.com/a/macros/fpsmail.org/s/AKfycbz5Q3pyjrdM-qxNsvGinRFv-pCMwevhMEYaniAJu00bIYeqNB5iF4eCNS6nBOmfZKBVFA/exec",
    maxReports: 250,
    expireDays: 7,
    fallbackReports: [
      { ID: 1, title: "Test Report", paragraph: "This is a fallback report.", created: new Date().toISOString(), type: "test", status: "open" }
    ]
  };
  ...
  /* ---------------------------
     Report Normalization & Filtering
  --------------------------- */
    ...
    return rows
      ...
      .filter(
        (r) =>
          r.id &&
          r.title &&
          daysSince(r.created) <= config.expireDays
      )
      .sort(
        (a, b) => Date.parse(b.created) - Date.parse(a.created)
      )
      .slice(0, config.maxReports);
  }
  ...
  function renderReports(reports) {
    ...
    if (!reports.length) {
      mount.innerHTML = "<p style='text-align:center;color:#aaa;'>No active reports.</p>";
      return;
    }
  ...
  async function loadReports() {
    try {
      ...
      try {
        ...
        console.warn("Fetch failed, using fallback reports", err);
        raw = config.fallbackReports;
      }
